$(function () {
  ...
  $('body').on('click', '.editar', function(){
    ...
    $('body').on('click' ,`.d-none${idClicado}`, function(){
      editarCliente(idClicado, $(`.editar-save${idClicado}`).val())
  ...
  function editarCliente(editarClienteId, nome){
    Swal.fire({
      title: 'Ateção?',
  ...
  $('body').on('click', '.deletar',  function(){
    deletarCliente($(this).data('id'))
  })

  function deletarCliente(deletarClienteId){
    Swal.fire({
      title: 'Ateção?',
      ...
      if (result.value) {
        $.ajax({
          url: '/api/deletar/cliente',
          method: 'DELETE',
          data:{'clienteId':deletarClienteId},
        }).done(function (data) {
          carregarTabela();
          Swal.fire({
            icon: 'success',
