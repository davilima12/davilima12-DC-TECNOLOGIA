$(function () {
  ...
  function carregarTabela() {
    $.ajax({
      ...
      var dt_ajax_table = $('.datatables-versao');
  ...
  $('body').on('click', '.editar', function(){
    ...
    $('body').on('click' ,`.d-none${idClicado}`, function(){
      editarVersao(idClicado, $(`.editar-save${idClicado}`).val())
  ...
  function editarVersao(editarVersaoId, nome){
    Swal.fire({
      title: 'Ateção?',
      ...
      if (result.value) {
        $.ajax({
          url: '/api/editar/versao',
          method: 'PUT',
          data:{
            'versaoId': editarVersaoId,
            'nome': nome
          },
        }).done(function (data) {
          carregarTabela();
  ...
  $('body').on('click', '.deletar',  function(){
    deletarVersao($(this).data('id'))
  })

  function deletarVersao(deletarVersaoId){
    Swal.fire({
      title: 'Ateção?',
      ...
      if (result.value) {
        $.ajax({
          url: '/api/deletar/versao',
          method: 'DELETE',
          data:{'versaoId':deletarVersaoId},
        }).done(function (data) {
          carregarTabela();
          Swal.fire({
            icon: 'success',
