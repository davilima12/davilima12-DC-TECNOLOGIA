$(function () {
  ...
  function carregarTabela() {
    $.ajax({
      ...
      var dt_ajax_table = $('.datatables-marca');
  ...
  $('body').on('click', '.editar', function(){
    ...
    $('body').on('click' ,`.d-none${idClicado}`, function(){
      editarMarca(idClicado, $(`.editar-save${idClicado}`).val())
  ...
  function editarMarca(editarMarcaId, nome){
    Swal.fire({
      title: 'Ateção?',
      ...
      if (result.value) {
        $.ajax({
          url: '/api/editar/marcas',
          method: 'PUT',
          data:{
            'marcaId': editarMarcaId,
            'nome': nome
          },
        }).done(function (data) {
          carregarTabela();
  ...
  $('body').on('click', '.deletar',  function(){
    deletarMarca($(this).data('id'))
  })

  function deletarMarca(deletarMarcaId){
    Swal.fire({
      title: 'Ateção?',
      ...
      if (result.value) {
        $.ajax({
          url: '/api/deletar/marca',
          method: 'DELETE',
          data:{'marcaId':deletarMarcaId},
        }).done(function (data) {
          carregarTabela();
          Swal.fire({
            icon: 'success',
